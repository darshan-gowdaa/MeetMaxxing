...
export default function UploadDialog({
	 ...
	 // Escape/scrim-closing is disabled while an upload is in flight
	 const panelRef = useDialogA11y<HTMLDivElement>({ onClose: onCancel, enabled: mounted && !busy });
	 const titleId ="upload-dialog-title";
	 ...
	 if (ext === 'pdf') return <RiFilePdfLine className="w-5 h-5 text-risk" aria-hidden="true"/>;
	 if (ext === 'docx') return <RiFileWordLine className="w-5 h-5 text-primary" aria-hidden="true"/>;
	 return <RiFileTextLine className="w-5 h-5 text-text-muted" aria-hidden="true"/>;
	 };

	 return createPortal(
	 <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
	 <div className="absolute inset-0 bg-bg/80"onClick={!busy ? onCancel : undefined} aria-hidden="true" />
	 <div
	 ref={panelRef}
	 role="dialog"
	 aria-modal="true"
	 ...
	 tabIndex={-1}
	 className="relative z-10 bg-surface-container-highest rounded-[28px] p-5 md:p-6 max-w-lg w-full border border-border animate-fade-scale shadow-sm focus:outline-none"
	 >
	 <div className="flex justify-between items-center mb-4">
	 <div className="w-10 h-10 rounded-full bg-primary-container flex items-center justify-center">
	 <RiUploadCloud2Line className="w-5 h-5 text-primary" aria-hidden="true"/>
	 </div>
	 <button onClick={onCancel} disabled={busy} aria-label="Close upload dialog" className="text-text-muted hover:text-text p-1 disabled:opacity-50">
	 <RiCloseLine className="w-5 h-5" aria-hidden="true"/>
	 </button>
	 </div>

	 <h2 id={titleId} className="text-[18px] font-bold text-text tracking-tight mb-1">
	 Upload Context
	 </h2>
