...
import { RiMailLine, RiLockPasswordLine, RiSparkling2Fill } from"@remixicon/react";
...
export default function LoginPage() {
 ...
 email, password, options: { emailRedirectTo: `${origin}/auth/callback?next=${encodeURIComponent(next)}` }
 });
 ...
 return (
 <div className="min-h-screen bg-bg flex flex-col md:flex-row text-text animate-fade-scale">
 {/* Hero Section */}
 ...
 <RiMailLine className="absolute left-4 top-1/2 /2 w-5 h-5 text-text-variant"/>
 <input type="email"required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email address"className="w-full h-14 bg-surface border border-border rounded-[16px] pl-12 pr-4 text-text placeholder:text-text-variant focus:outline-none focus:border-primary focus:bg-surface-container-high transition-colors spring-sm"/>
 </div>

 {mode !=="forgot"&& (
 <div className="relative">
 <RiLockPasswordLine className="absolute left-4 top-1/2 /2 w-5 h-5 text-text-variant"/>
 <input type="password"required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password"className="w-full h-14 bg-surface border border-border rounded-[16px] pl-12 pr-4 text-text placeholder:text-text-variant focus:outline-none focus:border-primary focus:bg-surface-container-high transition-colors spring-sm"/>
 </div>
 )}

 {error && <div className="text-on-risk bg-risk-container text-sm text-center py-3 rounded-[16px] font-medium">{error}</div>}
 {success && <div className="text-on-success-container bg-success-container text-sm text-center py-3 rounded-[16px] font-medium">{success}</div>}

 <button type="submit"disabled={loading} className="w-full h-14 bg-primary text-on-primary font-bold rounded-full hover:bg-primary-container hover:text-on-primary-container active:opacity-80 transition-colors disabled:opacity-50 mt-4 spring">
 {loading ?"Please wait...": (mode ==="signin"?"Sign in": mode ==="signup"?"Sign up":"Send reset link")}
 </button>
 </form>

 <div className="mt-8 flex flex-col items-center gap-3 text-sm font-bold">
 {mode ==="signin"? (
 <>
 <button onClick={() => setMode("forgot")} className="text-text-variant hover:text-primary transition-colors">Forgot password?</button>
 <button onClick={() => setMode("signup")} className="text-primary hover:text-primary-container transition-colors">Don&apos;t have an account? Sign up</button>
 </>
 ) : (
 <button onClick={() => setMode("signin")} className="text-primary hover:text-primary-container transition-colors">Back to sign in</button>
 )}
 </div>
 </div>
 </div>
